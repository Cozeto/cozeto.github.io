...
const NoteList: React.FC<NoteListProps> = ({ notes }) => {
  ...
  const [selectedTag, setSelectedTag] = useState<string | null>(getTagFromUrl());

  // Sync selectedTag with URL query param (in case of browser navigation)
  useEffect(() => {
    setSelectedTag(getTagFromUrl() || null);
  }, [location.search]);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-16 py-8">
      {/* Hero / Header Section */}
      <section className="flex items-center justify-center space-y-6 animate-in fade-in slide-in-from-top-4 duration-1000 min-h-[10vh]">
        <h1 className="text-6xl md:text-7xl font-extrabold tracking-tight text-gray-900 dark:text-white leading-[1.1] text-center">
          Notes.
        </h1>
      </section>
            ...
            {allTags.map(tag => (
              <button
                ...
                onClick={() => {
                  ...
                  if (newTag) {
                    navigate({ pathname: location.pathname, search: `?query=${encodeURIComponent(newTag)}` });
                  } else {
                    navigate({ pathname: location.pathname });
                  }
                }}
